...
public class AppScript {
    ...
    private final String appScriptUrl = "https://script.google" +
            ".com/macros/s/AKfycby3oR8IH5Z7uaTi-i_GPUfWeeFV96ejxfyo3dlq8tXZivpW51F0/exec";
    ...
    public List<Order> readOrders(@NotNull Sheet sheet) {
        ...
        for (int i = 0; i < com.amzass.utils.common.Constants.MAX_REPEAT_TIMES; i++) {
            try {
                ...
                logger.debug("基于AppScript读取{}下面{}条订单完毕, 耗时: {}", sheet.abbrev(), orders.size(), Tools.formatCostTime
                        (start));
                return orders;
    ...
    public boolean validateOrderId(String orderId) {
        return Regex.AMAZON_ORDER_NUMBER.isMatched(orderId) || RegexUtils.match(orderId, "#[A-Za-z0-9]*-[0-9]*") ||
                RegexUtils.match(orderId, "#[0-9]*");
    }
    ...
    private List<Order> parse(String json, @NotNull Sheet sheet) {
        ...
        String[] multiRows = StringUtils.split(StringUtils.defaultString(result.multiRows), com.amzass.utils.common
                .Constants.COMMA);
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            int maxCol = columns.length >= OrderColumn.TRACKING_NUMBER.number() ? OrderColumn.TRACKING_NUMBER.number
                    () : columns.length;
    ...
    Order readRefreshedOrder(String sheetUrl, String sheetName, Order order) {
        ...
        for (int i = 0; i < com.amzass.utils.common.Constants.MAX_REPEAT_TIMES; i++) {
            ...
            try {
                ...
                errorMsg = String.format("Failed to read latest data of %s via parsing AppScript response result %s",
                        order.id(), json);
            }
        }
    ...
    private <T> T readCertainColumns(String spreadId, String sheetName, Method method, Class<T> clazz) {
        ...
        try {
            ...
            throw new BusinessException(String.format("Failed to read %s-%s, %s in AppScript response result %s",
                    spreadId, sheetName, methodName, json));
        }
    }
    ...
    private String[][] readTrackNos(String spreadId, String sheetName, DataSourceCategory category) {
        ...
        throw new IllegalArgumentException("Current DataSource category is " + category + ", however only LAExport, " +
                "NJExport, UKForward and USForward are accepted.");
    }
    ...
    public List<TransferTrackingData> readTrackingData(String spreadId, String sheetName, DataSourceCategory category) {
        ...
        for (String[] array : data) {
            TransferTrackingData trackingData = new TransferTrackingData(spreadId, sheetName, array[0], array[1],
                    array[2]);
            if (array.length >= 9) {
    ...
    public void writeBuyerCancelLog(String spreadId, String sheetName, String orderId) {
        ...
        String data = today + com.amzass.utils.common.Constants.TAB + orderId + com.amzass.utils.common.Constants.TAB
                + Remark.BUYER_CANCELLED;
        if (!success) {
            throw new BusinessException(String.format("Failed to write memo of buyer cancel log after %s attempts: %s" +
                    ". Please paste it manually: %s.", com.amzass.utils.common.Constants.MAX_REPEAT_TIMES, error,
                    data));
        }
    }
    ...
    private void simpleOperation(int row, String sheetName, String spreadId, Method method, String... values) {
        ...
        if (!success) {
            throw new BusinessException(String.format("Failed to perform %s for order at row %s in google sheet %s " +
                    "after %s attempts: %s. Please try to do it manually.", method.name(), row, sheetName, com.amzass
                    .utils.common.Constants.MAX_REPEAT_TIMES, StringUtils.defaultString(json)));
        }
    }
    ...
    public int findRowByOrderId(String orderId, String sheetName, String spreadId) {
        ...
        if (rc == -1 || rc < 2) {
            throw new OrderNotFoundException(String.format("Failed to find row number in google sheet %s according to" +
                    " order id %s. AppScript response result: %s", sheetName, orderId, text));
        }
        return rc;
    }
    ...
    public int findRowByOrderNoAndId(String orderNo, String orderId, String sheetName, String spreadId) {
        if (StringUtils.isBlank(orderNo) || StringUtils.isBlank(orderId)) {
            throw new IllegalArgumentException(String.format("Order id %s or supplier order number %s is empty, which" +
                    " is not allowed.", StringUtils.defaultString(orderId), StringUtils.defaultString(orderNo)));
        }
        ...
        if (rc == -1 || rc < 2) {
            throw new OrderNotFoundException(String.format("Failed to find row number in google sheet %s according to" +
                    " order id %s and supplier order number %s. AppScript response result: %s", sheetName, orderId,
                    orderNo, text));
        }
        return rc;
    }
    ...
    public void markRefunded(int row, String sheetName, String spreadId) {
        this.simpleOperation(row, sheetName, spreadId, Method.UpdateColumnAD, "Fully refunded and notification letter" +
                " sent");
    }
    ...
    public String markSellerCancel(int row, String sheetName, String spreadId, String cancelSheetName, String
            cancelSpreadId, boolean copy2CancelSheet, boolean ignoreExistence) {
        Map<String, String> params = new HashMap<>();
        ...
        throw new BusinessException(String.format("Failed to perform %s for order at row %s in google sheet %s after " +
                "%s attempts: %s. Please try to do it manually.", Method.MARKSELLERCANCEL.name(), row, sheetName, com
                .amzass.utils.common.Constants.MAX_REPEAT_TIMES, StringUtils.defaultString(json)));
    }
    ...
    public long getLastUpdateTime(String spreadId) {
        ...
        for (int i = 0; i < com.amzass.utils.common.Constants.MAX_REPEAT_TIMES; i++) {
            try {
                ...
                logger.debug("成功获取Spreadsheet {}的最后修改时间:{}，耗时{}", spreadId, DateHelper.toDateTime(new Date(timestamp)
        ...
        logger.error("经过{}次尝试仍无法获取{}对应Spreadsheet的最后修改时间", com.amzass.utils.common.Constants.MAX_REPEAT_TIMES,
                spreadId);
    ...
    public Sheets getSheets(String spreadId) {
        ...
        throw new BusinessException("Cannot get spreadsheet(ID: " + spreadId + ") metadata from response: '" +
                StringUtils.defaultString(json) + "'");
    }
     ...
     * 记录验证码处理结果
     */
    public void recordCaptchaResolveLog(String application, String accountType, String email, String url, String
            handlerId, boolean success) {
        try {
            Preconditions.checkArgument(Tools.isNotBlank(email, url));
            Map<String, String> params = new HashMap<>();
            params.put("application", application);
    ...
    public String uploadMailManConfig() {
        ...
        Cnd cnd = Cnd.where("context", com.amzass.utils.common.Constants.EQUALS, settings.context()).and("beginTime",
                ">=", DateHelper.beginOfDay(new DateTime()).getMillis() - PageLoadHelper.WaitTime.Long.valInMS()).and
                ("endTime", "<", DateHelper.endOfDay(new DateTime()).getMillis());
    ...
    private <T extends BaseStatisticSummary> String abbrevStatistic(List<T> statistics) {
        ...
        String warning = fail >= BaseStatisticSummary.FAIL_THRESHOLD_COUNT || (total > 0 && fail * 100.0f / total >=
                BaseStatisticSummary.FAIL_THRESHOLD_RATE) ? ("!!WARNING!!" + StringUtils.LF) : StringUtils.EMPTY;
        return String.format("%sTotal: %s%nSuccess: %s%nFail: %s%nSkip: %s%nFailure Rate: %s", warning, total,
                success, fail, skip, (total > 0 ? com.amzass.utils.common.Constants.DOUBLE_FORMAT.format(fail *
                        100.0f / total) + com.amzass.utils.common.Constants.PERCENTAGE : com.amzass.utils.common
                        .Constants.HYPHEN));
    }
    ...
    private String exec(String params4Url) {
        ...
        for (int i = 0; i < com.amzass.utils.common.Constants.MAX_REPEAT_TIMES; i++) {
            ...
            if (SUCCESS.equals(json) || Tools.startWithAny(json, SUCCESS)) {
                logger.debug("{}已成功执行, 结果:{}", PageUtils.decodeUrl(params4Url), StringUtils.abbreviate(json, com
                        .amzass.utils.common.Constants.MAX_MSG_LENGTH * 2));
                return json;
            }
        }
        throw new BusinessException(String.format("Failed to invoke '%s' after %s attempts. Error detail:%n%s.",
                StringUtils.abbreviate(params4Url, com.amzass.utils.common.Constants.MAX_URL_LENGTH), com.amzass
                        .utils.common.Constants.MAX_REPEAT_TIMES, StringUtils.defaultString(json)));
    }
